# 💻 Código ampliado en Python ECOMAKET
...
class Inventario:
    ...
    def registrar_producto(self, codigo, nombre, categoria, cantidad_inicial, ubicacion=None, precio_unitario=0.0):
        ...
        return {
            ...
            "resumen": {
                ...
                "Precio Unitario": f"S/{precio_unitario:.2f}"
            }
        }
    ...
    def calcular_valor_total_inventario(self, umbral_valor=10000.0, recargo_porcentaje=0.08):
        ...
        if valor_total_sin_recargo > umbral_valor:
            ...
            valor_final += recargo
            mensaje_recargo = f"¡Se aplicó un recargo del {recargo_porcentaje*100:.0f}% (S/{recargo:.2f}) porque el valor supera el umbral de S/{umbral_valor:.2f}!"

        return (f"--- CÁLCULO DE VALOR TOTAL DEL INVENTARIO ---\n"
                f"Valor total del inventario (sin recargo): S/{valor_total_sin_recargo:.2f}\n"
                f"{mensaje_recargo}\n"
                f"Valor total final del inventario: S/{valor_final:.2f}")
...
def ejecutar_inventario_interactivo():
    ...
    while True:
        print("\n==== Sistema de Gestión de Inventario ECOMARKET ====")
        print("")
